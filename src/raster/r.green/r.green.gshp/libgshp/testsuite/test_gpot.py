        ...
        for i_row, (r_row, a_row) in enumerate(zip(ref, act)):
            ...
            if diff.any():
                ...
                raise AssertionError(
                    msg.format(i_row=i_row, cols=cols, ref=r_row[cols], act=a_row[cols])
                )
...
class TestBHEresistence(TestCase):
    def test_resistence(self):
        ...
        res = gpot.get_borehole_resistence(
...
class TestGPot(TestCase):
    def test_norm_time(self):
        ...
        for time, result in zip(
            (HEATING_SEASON, LIFETIME), (0.00011302806712962963, 1.1147973744292237e-06)
        ):
            res = gpot.norm_time(
                time,
                borehole_radius=BH_RADIUS,
...
class TestRasterGPot(TestCase):
    ...
    def setUpClass(cls):
        ...
        call_module(
            "r.in.ascii", input=cls.gmax_ref_file, output=cls.gmax_ref, overwrite=True
        )
        call_module(
            "r.in.ascii", input=cls.power_ref_file, output=cls.power_ref, overwrite=True
        )
        call_module(
            "r.in.ascii",
    ...
    def test_norm_time_hs(self):
        ...
        assert_raster_no_difference(
            actual=self.norm_time_hs_comp,
    ...
    def test_norm_thermal_alteration(self):
        ...
        gpot.r_norm_thermal_alteration(
            out=self.gmax_comp,
            tc=HEATING_SEASON / (365.0 * 24 * 60 * 60),
    ...
    def test_power(self):
        ...
        gpot.r_power(
            ...
            fluid_limit_temperature=-2.0,
            borehole_length=100.0,
            borehole_resistence=0.1,
            gmax=self.gmax_ref,
            execute=True,
            overwrite=True,
        )
    ...
    def test_energy(self):
        ...
        gpot.r_energy(
            out=self.energy_comp, power=self.power_ref, execute=True, overwrite=True
        )
        assert_raster_no_difference(
            actual=self.energy_comp, reference=self.energy_ref, precision=self.precision
        )
